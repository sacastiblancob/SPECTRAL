...
epsilons = [0.05 0.001 0.00025];

nn = length(Nes);
nk = length(Kes);
ne = length(epsilons);

RES = -999*ones(nn*nk*ne,107);
...
%         toc
    ...
    RES(o,4) = dt;
    RES(o,5) = toc;
    RES(o,6) = norm(sum(Eto)-Ebala);
    RES(o,7) = norm(sum(Eto)-Ebal);
    RES(o,8) = norm(sum(Eto)-Ebala,'Inf');
    RES(o,9) = norm(sum(Eto)-Ebal,'Inf');
    RES(o,10) = sum(Ebala-sum(Eto))/length(Ebala);
    RES(o,11) = sum(Ebal-sum(Eto))/length(Ebal);
