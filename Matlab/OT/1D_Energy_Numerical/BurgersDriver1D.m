...
Kes = 1:16;
% epsilons = [0.05 0.025 0.01 0.0075 0.0050 0.0025 0.001 0.00075 0.0005 0.00025];
...
RES = -999*ones(nn*nk*ne,55);

% N ; K ; NU ; norm2(ana) ; norm2(num) ; norminf(ana) ; norminf(num) ;
% mae(ana) ; mae(num) ; norm2(ana) ele ; norm2(num)ele ; norminf(ana) ele ; norminf(num) ele ;
% mae(ana) ele ; mae(num) ele
...
%     N = 12;
    ...
    epsilon = epsilons(kk);      %No need of filtering
%     epsilon = 0.00025;
    % epsilon = 0.00025;      %viscosity constant (need of filtering and works)
...
%       dt = 0.1*CFL*(xmin/umax);
      %dt = 0.1*CFL*(xmin/umax);
      %dt = 0.005;
      display([ii jj kk o dt N K epsilon])
%       if dt < 8E-5
%           RES(o,1) = N;
%           RES(o,2) = K;
%           RES(o,3) = epsilon;
...
%       shapestr = { '-o','-x' };
    ...
    %             end
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%               if INTRK ~= 5
%                 ua = analitica(x,time + rk4c(INTRK+1)*dt,epsilon,etai,wi,ai);
%               else
%                 ua = analitica(x,time+dt,epsilon,etai,wi,ai);
%               end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % energy calculations
              Calc_energy             
...
%         toc
    ...
    % Eto is the initial energy by element

    Ebal = E+Ev+Evf+Enlf;

%     Ebala = Ea+Eva+Evfa+Enlfa;   %(analytical)

    Econ = EEt + dEEt + dfEEt + nfEEt;

%     Econa = EEta + dEEta + dfEEta + nfEEta;     %(analytical)
    
    norm2 = norm(sum(Eto)-Ebal);
    norminf = norm(sum(Eto)-Ebal,'Inf');
    MAE = sum(Ebal-sum(Eto))/length(Ebal);
    RES(o,1) = N;
    RES(o,2) = K;
    RES(o,3) = epsilon;
    RES(o,4) = toc;
    RES(o,5) = norm(sum(Eto)-Ebal);
    RES(o,6) = norm(sum(Eto)-Ebal,'Inf');
    RES(o,7) = sum(Ebal-sum(Eto))/length(Ebal);

%     % analytical
%     norm2 = norm(sum(Eto)-Ebala);
%     norminf = norm(sum(Eto)-Ebala,'Inf');
%     MAE = sum(Ebala-sum(Eto))/length(Ebala);
%     RES(o,1) = N;
%     RES(o,2) = K;
%     RES(o,3) = epsilon;
%     RES(o,4) = toc;
%     RES(o,5) = norm(sum(Eto)-Ebala);
%     RES(o,6) = norm(sum(Eto)-Ebala,'Inf');
%     RES(o,7) = sum(Ebala-sum(Eto))/length(Ebala);


    mm=8;
    for n=1:K
        RES(o,mm) = norm(Eto(n)-Econ(:,n));
        mm=mm+1;
    end
    for n=1:K
        RES(o,mm) = norm(Eto(n)-Econ(:,n),'Inf');
        mm=mm+1;
    end
    for n=1:K
        RES(o,mm) = sum(Econ(:,n)-Eto(n))/length(Econ(:,n));
        mm=mm+1;
    end
...
%     mm=8;
%     for n=1:K
%         RES(o,mm) = norm(Eto(n)-Econa(:,n));
%         mm=mm+1;
%     end
%     for n=1:K
%         RES(o,mm) = norm(Eto(n)-Econa(:,n),'Inf');
%         mm=mm+1;
%     end
%     for n=1:K
%         RES(o,mm) = sum(Econa(:,n)-Eto(n))/length(Econa(:,n));
%         mm=mm+1;
%     end

    %clearvars -except RES
    display([mm toc RES(o,7)])
