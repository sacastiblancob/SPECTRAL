...
for time in range(1,Ts):
    Cm1 = -(D*K**2 + r + 1j*u*K)*Com
    Cm2 = -(D*K**2 + r + 1j*u*K)*(Com+Cm1*(dt/2))
    Cm3 = -(D*K**2 + r + 1j*u*K)*(Com+Cm2*(dt/2))
    Cm4 = -(D*K**2 + r + 1j*u*K)*(Com+Cm3*dt)
    Cm = Com + (dt/6)*(Cm1 + 2*Cm2 + 2*Cm3 + Cm4)
    C[:,time] = IFT(Cm,N)
