...
b=0
for h in np.array((20,50,100,150,200,400,800,1200),dtype=np.int):
    #Time grid
    to = 2
    tf = 2.5
    t = np.linspace(to,tf,h)
    Ts = np.size(t)
    dt = t[1] - t[0]
    DT[b] = dt
    b = b+1
    ...
    #print(error[:,Ts-1])
#
#   Energy vs Time for diferent time step sizes 
#    plt.clf()
    fig1 = plt.subplot(1, 1, 1)
    fig1.set_xlim(t[0], t[Ts-1])
    fig1.set_ylim(0,0.5)
    fig1.set_xlabel(r't')
    fig1.set_ylabel(r'E')
    fig1.set_title('Time Energy Variation')
    plot1 = fig1.semilogy(t,E)

    plt.draw()
    plt.pause(0.1)
#definir las etiquetas de las curvas...
